-- Revert customer_type to company/individual (customer master)
-- Lead type → channel catalog: end_user · dealer · contractor · distributor · oem · other
-- Docs: CUSTOMER-MASTER-FIELDS.md §0 · LEADS-MODULE.md § Lead type
...
SET customer_type = 'company', updated_at = now()
WHERE customer_type IN ('end_user', 'dealer', 'contractor', 'distributor', 'oem');

UPDATE public.companies
SET customer_type = 'individual', updated_at = now()
WHERE customer_type = 'other';

ALTER TABLE public.companies
  ALTER COLUMN customer_type SET DEFAULT 'company';
...
WHERE customer_type = 'individual'
  ...
  AND (industry_segment IS NOT NULL OR industry IS NOT NULL);
...
ALTER TABLE public.companies
  ...
  CHECK (
    ...
    OR (industry_segment IS NULL AND industry IS NULL)
  );

-- ── customer_type (leads snapshot) ───────────────────────────────────────────
...
UPDATE public.leads
SET lead_type = 'other', updated_at = now()
WHERE lead_type IN ('inbound', 'outbound', 'referral', 'partner');

ALTER TABLE public.leads
  ALTER COLUMN lead_type SET DEFAULT 'end_user';

ALTER TABLE public.leads
  ADD CONSTRAINT leads_lead_type_check
  CHECK (lead_type IN ('end_user', 'dealer', 'contractor', 'distributor', 'oem', 'other'));
...
BEGIN
  ...
  INSERT INTO public.leads (
    ...
    coalesce(nullif(trim(p_payload->>'lead_type'), ''), 'end_user'),
    nullif(trim(p_payload->>'owner_id'), '')::uuid,
...
BEGIN
  ...
  SET
    ...
    lead_type = coalesce(nullif(trim(p_payload->>'lead_type'), ''), 'end_user'),
    owner_id = nullif(trim(p_payload->>'owner_id'), '')::uuid,
