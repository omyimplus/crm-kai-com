-- Tasks — optional sales team assignment
-- Docs: TASKS-MODULE.md · tables.md

ALTER TABLE public.tasks
  ADD COLUMN IF NOT EXISTS sales_team_id uuid REFERENCES public.sales_teams(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_tasks_org_sales_team
  ON public.tasks (org_id, sales_team_id)
...
-- Return type changed (sales_team_id, sales_team_name) — must drop before recreate
...
BEGIN
  ...
  LEFT JOIN public.sales_teams st ON st.id = t.sales_team_id AND st.deleted_at IS NULL
...
BEGIN
  ...
  v_sales_team_id := nullif(trim(p_payload->>'sales_team_id'), '')::uuid;
  IF v_sales_team_id IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM public.sales_teams st
