-- Remove employee position master — org uses Setup → system users instead

DROP FUNCTION IF EXISTS public.create_employee_position(jsonb);
DROP FUNCTION IF EXISTS public.update_employee_position(uuid, jsonb);
DROP FUNCTION IF EXISTS public.soft_delete_employee_position(uuid);
DROP FUNCTION IF EXISTS public.restore_employee_position(uuid);
DROP FUNCTION IF EXISTS public.employee_position_log_snapshot(uuid);
DROP FUNCTION IF EXISTS public.employee_position_deleted_snapshot(uuid);
...
RETURNS text[] AS $$
  SELECT ARRAY[
    ...
    'master.category', 'master.leadSource', 'master.partner', 'master.unit',
