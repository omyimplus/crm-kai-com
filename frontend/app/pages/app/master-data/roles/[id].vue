...
async function load() {
  ...
  try {
    ...
    if (!role.value) {
      await navigateTo('/app/master-data/roles')
      return
    }
...
async function save() {
  ...
  try {
    ...
    savedMsg.value = t('masterData.roles.manage.saved')
    await load()
  } catch (e) {
...
<template>
  <div>
    <UButton
      ...
      class="mb-4"
    >
      {{ t('masterData.roles.manage.back') }}
    </UButton>
      ...
      <div class="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
            {{ t('masterData.roles.manage.subtitle') }}
          </p>
        </div>
        ...
        <div class="flex flex-wrap gap-2">
          ...
          >
            {{ t('masterData.roles.manage.assignUsers') }}
          </UButton>
          <UButton
            :loading="saving"
            icon="i-lucide-save"
      ...
      <div class="mb-6 grid gap-4 sm:grid-cols-3">
        <UCard>
          <p class="text-xs font-medium uppercase tracking-wide text-gray-500">
            {{ t('masterData.roles.permissions.count') }}
          </p>
        ...
        <UCard>
          <p class="text-xs font-medium uppercase tracking-wide text-gray-500">
            {{ t('masterData.roles.manage.modules') }}
          </p>
          <p class="mt-1 text-2xl font-bold font-heading">
      ...
      <UCard class="mb-6">
        <template #header>
          <h2 class="font-semibold font-heading">
            {{ t('masterData.roles.manage.details') }}
          </h2>
        </template>
        ...
        >
          ...
          <UFormField
            :label="t('masterData.roles.descriptionLabel')"
            class="md:col-span-2"
          >
            <UTextarea
              v-model="description"
      ...
      <UCard>
        <template #header>
          <div class="flex flex-wrap items-center justify-between gap-2">
            <h2 class="font-semibold font-heading">
              {{ t('masterData.roles.permissions.title') }}
            </h2>
            <p class="text-sm text-gray-500">
              {{ t('masterData.roles.permissions.granted', { count: grantedCount }) }}
            </p>
          </div>
        </template>
