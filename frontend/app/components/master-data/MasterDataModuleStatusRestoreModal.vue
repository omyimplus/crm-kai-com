...
const { t } = useI18n()
const { restore } = useModuleStatuses()
...
async function confirmRestore() {
  ...
  errorMsg.value = ''
  try {
    await restore(props.moduleStatusId)
    open.value = false
    emit('restored')
  } catch (e: unknown) {
...
<template>
  <AppDialog
    v-model:open="open"
    :title="t('masterData.moduleStatuses.restoreTitle')"
    size="sm"
  >
    <p class="text-sm leading-relaxed text-gray-600 dark:text-gray-400">
      {{ t('masterData.moduleStatuses.restoreConfirm', { name: moduleStatusName }) }}
    </p>
