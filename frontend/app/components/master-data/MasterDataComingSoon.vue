...
const props = defineProps<{
  menuKey: MasterDataMenuKey
}>()

const { t, tm } = useI18n()

const item = computed(() => masterDataMenuItems.find(i => i.key === props.menuKey)!)
const title = computed(() => t(`masterData.${props.menuKey}.title`))
const description = computed(() => t(`masterData.${props.menuKey}.description`))
const features = computed(() => {
  const raw = tm(`masterData.${props.menuKey}.features`) as string[] | string
