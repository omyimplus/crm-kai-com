...
import { partnerDeleteErrorMessage } from '~/utils/masterPartner'
...
async function confirmDelete() {
  ...
  errorMsg.value = ''
  try {
    await remove(props.partnerId)
    open.value = false
    emit('deleted')
  } catch (e: unknown) {
    errorMsg.value = partnerDeleteErrorMessage(e, t)
...
<template>
  <AppDialog
    v-model:open="open"
    :title="t('masterData.partner.deleteTitle')"
    size="sm"
  >
    <p class="text-sm leading-relaxed text-gray-600 dark:text-gray-400">
      {{ t('masterData.partner.deleteConfirm', { name: partnerName }) }}
    </p>
    <p class="mt-3 text-sm leading-relaxed text-gray-500 dark:text-gray-400">
      {{ t('masterData.partner.deleteSoftHint') }}
    </p>
